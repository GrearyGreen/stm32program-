...
{  
	GPIO_InitTypeDef GPIO_InitStructure;					//管脚复用
	TIM_TimeBaseInitTypeDef  TIM_TimeBaseStructure;			//TIM3结构体
	TIM_OCInitTypeDef  TIM_OCInitStructure;					//定时器通道
	

	RCC_APB1PeriphClockCmd(RCC_APB1Periph_TIM3, ENABLE);	//使能定时器3时钟
 	RCC_APB2PeriphClockCmd(RCC_APB2Periph_GPIOB  | RCC_APB2Periph_AFIO, ENABLE);  //使能GPIO外设和AFIO复用功能模块时钟
	
//    GPIO_PinRemapConfig(GPIO_Remap_SWJ_JTAGDisable, ENABLE);//使能JTAGDisable，即禁用JTAG接口,因为需要映射PB4
...
//    GPIO_PinRemapConfig(GPIO_Remap_SWJ_JTAGDisable, ENABLE);//使能JTAGDisable，即禁用JTAG接口,因为需要映射PB4
	...
	GPIO_PinRemapConfig(GPIO_Remap_TIM4, DISABLE); //Timer3部分重映射  TIM3_CH2->PB5    
